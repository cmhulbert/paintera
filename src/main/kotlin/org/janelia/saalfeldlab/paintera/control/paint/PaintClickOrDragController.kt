...
class PaintClickOrDragController(
    ...
    fun submitPaint() {
        synchronized(this) {
            when {
                ...
                !submitMask -> LOG.debug("submitMask flag: $submitMask")
                else -> try {
                    with(paintIntoThis!!) {
                        viewerMask?.let {
                            val repaintInterval = extendAndTransformBoundingBox(sourceInterval!!.smallestContainingInterval, it.labelToGlobalTransform, 0.5)
    ...
    internal var sourceInterval: RealInterval? = null
    ...
    private fun getViewerMipMapLevel(): Int {
    ...
    private fun paint(viewerX: Double, viewerY: Double) {
        ...
        viewerMask?.run {
            ...
            val (initialX, initialY) = currentToInitialPoint(viewerX, viewerY)
            ...
            val sourcePaintInterval = extendAndTransformBoundingBox(paintIntervalInCurrentViewer, labelToViewerTransform.inverse(), .5)
            val globalPaintInterval = extendAndTransformBoundingBox(sourcePaintInterval, labelToGlobalTransform, .5)


            viewerInterval = paintIntervalInCurrentViewer union viewerInterval
            sourceInterval = sourcePaintInterval union sourceInterval
    ...
    private fun ViewerMask.currentToInitialPoint(viewerX: Double, viewerY: Double): Pair<Double, Double> {
        val pointInInitialViewerSpace = doubleArrayOf(viewerX, viewerY, 0.0).also {
            initialToCurrentViewerTransform.applyInverse(it, it)
        }
        return pointInInitialViewerSpace[0] to pointInInitialViewerSpace[1]
    }

    private fun ViewerMask.initialBrushRadius(): Double {
        val globalToInitialViewerTransform = initialToCurrentViewerTransform.copy().inverse().concatenate(globalToViewerTransform)
