...
import org.janelia.saalfeldlab.fx.extensions.createNonNullProperty
...
internal class SmoothLabelUI(val model: Model) : VBox(10.0) {
	...
	interface Model {
		...
		val canApply: Property<Boolean>
		val canCancel: Property<Boolean>
		...
		fun getDialog(title: String = "Smooth Label") = Dialog<Boolean>().apply {
			...
			dialogPane.lookupButton(ButtonType.APPLY).also { applyButton ->
				applyButton.disableProperty().bind(canApply.createNonNullValueBinding { !it })
			...
			cancelButton.disableProperty().bind(canCancel.createNonNullValueBinding { !it })
	...
	class Default : Model {
		...
		override val canApply: Property<Boolean> = statusProperty.createNonNullProperty { it == SmoothStatus.Done }
		override val canCancel: Property<Boolean> = statusProperty.createNonNullProperty { it != SmoothStatus.Applying }
