...
public class LabelSourceState<D extends IntegerType<D>, T extends Volatile<D> & Type<T>>
  ...
  @Override public List<ActionSet> getGlobalActionSets() {
	...
	var labelSourceStateGlobalActionSet = new ActionSet(LabelSourceStateKeys.CANCEL_3D_FLOODFILL, actionSet -> {
	  ...
	  actionSet.addKeyAction(KeyEvent.KEY_PRESSED, action -> {
		//TODO Caleb: Move CANCEL_3D_FLOODFILL to the PaintLabelTool, since we can only trigger flood fill 3d from there.
		action.setConsume(true);
		action.keyMatchesBinding(keyBindings, LabelSourceStateKeys.CANCEL_3D_FLOODFILL);
		action.verify(event -> {
		  FloodFillState state = this.floodFillState.get();
		  return state != null && state.interrupt != null;
		});
		action.onAction(event -> this.floodFillState.get().interrupt.run());
	  });
